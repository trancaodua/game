                ...
                <div v-for="item in data" :key="item.key" class="mini-carousel-item-container">
                    <div class="mini-carousel-item" :class="[item.class]" v-bind:style="{ 'background-image': 'url(' + gameType.image + ')'}">
                        ...
                        <div class="mini-carousel-body">
                            ...
                            <div class="mini-carousel-button-bet">Cược ngay</div>
                        </div>
                    </div>
                </div>
            </div>
            <div @click="prev" class="mini-carousel-button left" id="left"></div>
            <div @click="next" class="mini-carousel-button right" id="right"></div>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        miniChangeImage() {
            if (this.miniIdx > this.data.length - 1) {
                this.miniIdx = 0;
            }
            else if (this.miniIdx < 0) {
                this.miniIdx = this.data.length - 1;
            }
            this.transform = `translateX(${-this.miniIdx * this.miniImgs.offsetWidth}px)`;
        },
        next() {
            this.miniIdx++;
            this.miniChangeImage();
...
.mini-carousel-item.two {
    background-position: -0.142em -59.984em;
}

.mini-carousel-item.three {
    background-position: -0.142em -40.037em;
}

.mini-carousel-item.four {
    background-position: -0.142em -20.09em;
}

.mini-carousel-tag-hot {
    width: 2.425em;
    height: 2.425em;
...
.mini-carousel-debet span {
    font-size: 1.167em;
}

.mini-carousel-button-bet {
    display: inline-block;
    width: 6em;
    height: 1.692em;
    background: #ffa240;
    box-shadow: 0 1px 2px 0 rgba(12, 33, 89, 0.28);
    border-radius: 0.784em;
    font-size: 0.926em;
    font-weight: 400;
    color: #fff;
    line-height: 1.692;
...
.mini-carousel-button {
    ...
    background-image: url(data:image/png;base64,);
}

.mini-carousel-button.left {
    left: 0;
    transform: rotate(180deg);
}

.mini-carousel-button.right {
    right: 0;
}
</style>
